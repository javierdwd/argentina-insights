...
- Start with the most specific standard catalog widget. Its data binding,
  normalization, interaction, accessibility, and tested responsive behavior
  are concrete benefits that Box gives up. Choose Box only when its functional
  benefit clearly outweighs all of those costs: the requested relationship or
  named visual form cannot be represented honestly by any standard widget.
  Novelty, custom styling, or a different card layout are not benefits by
  themselves. If the tradeoff is uncertain, use the standard widget.
  A profile or roster of people MUST use PersonCard whenever
  person rows are available. Never recreate a person's name, role, party,
  province, vote, photo, bio, or contact fields in Box, Text, or Callout.
  Box is reserved for genuinely bespoke semantic tables, diagrams, flows,
  hierarchies, matrices, or relationships that would lose essential meaning
  in every standard widget.
  The visible "generated by AI" treatment raises the quality bar: Box must
  deliver an immediately legible visual or structural gain. Never use Box for
  a generic card, a heading plus paragraph, a single prose block, or styled
  text; use Text or Callout instead. Attractive means useful hierarchy and
  composition—not decoration, ornamental SVG, or extra chrome.
  ...
  A Box title is only chrome, never content: Box must contain authored children
  that communicate sourced facts. If no useful fact is available, leave the
  canvas unchanged and explain the limitation in `brief`; never echo requested
  field names, implementation instructions, or missing-data placeholders.
...
def compose_ui_node(state: AgentState) -> dict:
    ...
    llm = get_model(role)
    # UINode's flexible props schema is not accepted by OpenAI's strict
    # response_format subset, so composition must use tool-based structuring.
