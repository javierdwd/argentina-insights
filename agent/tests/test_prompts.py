...
def test_compose_catalog_requires_box_benefits_to_outweigh_costs() -> None:
    catalog = widget_catalog_text().casefold()
    assert "### box" in catalog
    assert "svg" in catalog
    assert "functional benefit clearly outweighs" in catalog
    assert "when uncertain, use the standard widget" in catalog
    assert "heading plus paragraph" in catalog
    ...
    assert "any single-person profile or people roster" in catalog
    compose = _COMPOSE_SYSTEM.casefold()
    assert "box" in compose
    assert "substitute" in compose and "data-bound widget" in compose
    assert "benefit clearly outweighs all" in compose
    assert "if the tradeoff is uncertain" in compose
    assert "must use personcard" in compose
    assert "visible \"generated by ai\" treatment" in compose
    assert "a heading plus paragraph" in compose
    assert "semantic tables" in compose
